...
$(document).ready(function(){
  $('#file_upload').uploadify({
    ...
    'folder'    : '<?php echo $path; ?>images/items',
    'auto'      : false,
    'onComplete'  : function(event, ID, fileObj, response, data) {
      $("#picture").val(fileObj['name']);
      $("#thumb").val(fileObj['name']);
      $("#display_picture").html("<img src='<?php echo $path; ?>images/items/Thumbs/"+fileObj['name']+"' height='52px' weight='50px'/>");
  ...
  $('#gallery_upload').uploadify({
    ...
    'script'    : '<?php echo base_url();?>js/uploadify-v2.1.4/uploadify.php',
    'cancelImg' : '<?php echo base_url();?>js/uploadify-v2.1.4/cancel.png',
    'folder'    : '<?php echo $path; ?>images/picture_items',
    'auto'      : false,
    'multi'     : true,
    'onComplete'  : function(event, ID, fileObj, response, data) {
  ...
  var options = { 
    ...
    $("#btnEdit").click(function(){
      ...
      if(id == "" || id == "undefine") {
        ...
        var href="<?php echo base_url();?>/auction_items_backend/edit/"+nextid[1];
        $(this).attr("href",href);
      }
    });
  ...
  $(".del_gallery").click(function(){
    ...
    $.ajax({
      url:"<?php echo base_url();?>auction_items_backend/delete_picture",
      data:'id='+id[1],
  ...
  $("#btnDel").click(function(){
    ...
    if(id!=""){
      var nextid=id.split("_");
      $.ajax({
        url: "<?php echo base_url();?>auction_items_backend/del",
        data:"id="+nextid[1],
        success: function(response){
          if(response=="success"){
            $("#"+id).hide("slow");  
  ...
  $("#btnDet").click(function(){
        ...
        if(id == "" || id == "undefine") {
          ...
          var nextid=id.split("_");
          var href="<?php echo base_url();?>/auction_items_backend/detail/"+nextid[1];
          $(this).attr("href",href);
        }
      });
});
